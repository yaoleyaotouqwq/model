...
def train_data_process(data,table_name):
    x_data = []
    y_data = []
    ...
    if isinstance(data,dict):
        ...
        if table_name == GUIPreDefine.Table_Name[0]:
            for temp in use_data:
                if temp[20] in Data.Score_dict:
                    temp_x = list(temp[:8]) + list(temp[9:17]) + list(temp[18:20]) + [Data.Score_dict[temp[20]]] + list(temp[21:])
                else:
                    temp_x = list(temp[:8]) + list(temp[9:17]) + list(temp[18:])
        ...
        if isinstance(data[0], list):
            ...
            if table_name == GUIPreDefine.Table_Name[0]:
                for temp in data:
                    if temp[20] in Data.Score_dict:
                        temp_x = list(temp[:8]) + list(temp[9:17]) + list(temp[18:20]) + [
                            Data.Score_dict[temp[20]]] + list(temp[21:])
                    else:
                        temp_x = list(temp[:8]) + list(temp[9:17]) + list(temp[18:])
                    temp_y = [temp[8]]
...
def predict_data_process(data,table_name):
    x_data = []
    y_data = []
